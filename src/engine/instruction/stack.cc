...
uint8_t PHA::Execute(uint8_t opcode, uint16_t operand) {
  bus_.Write(kStackStartAddress + reg_.sp--, reg_.acc);
  return 0;
}

uint8_t PLA::Execute(uint8_t opcode, uint16_t operand) {
  bus_.Read(kStackStartAddress + ++reg_.sp, &reg_.acc);
  return 0;
}

uint8_t PHP::Execute(uint8_t opcode, uint16_t operand) {
  bus_.Write(kStackStartAddress + reg_.sp--, reg_.status.to_ulong());
  return 0;
}

uint8_t PLP::Execute(uint8_t opcode, uint16_t operand) {
  uint8_t byte_read;
